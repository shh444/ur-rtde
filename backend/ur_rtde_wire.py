...
class RTDEConnection:
    ...
    def pause(self, print_text_messages: bool = True, timeout_s: float = 1.0) -> bool:
        ...
        while True:
            ...
            if packet_type == PKT_TEXT_MESSAGE:
                if print_text_messages:
                    ...
                    print(f"[RTDE TEXT_MESSAGE] lvl={level} src={source} msg={message}")
                if time.time() - started_at > timeout_s:
                    raise RTDEError("PAUSE timed out after only TEXT_MESSAGE packets")
                continue
            if packet_type == PKT_DATA_PACKAGE:
                if time.time() - started_at > timeout_s:
                    raise RTDEError("PAUSE timed out while draining DATA_PACKAGE packets")
                continue
            if packet_type != PKT_CONTROL_PAUSE:
                raise RTDEError(f"unexpected reply to PAUSE: {packet_type}")
            return bool(payload and payload[0] == 1)
