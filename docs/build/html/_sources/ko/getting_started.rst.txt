...
순서를 다룹니다. 화면별 사용법은 :doc:`shipyard_guide` 를, 레거시 통합 대시보드
(포트 8008)는 :doc:`dashboard_guide` 를 참고하세요.
...
.. list-table::
   ...
   * - shipyard (권장)
     - 8010
     - 조선소 용접 워크플로 전용. GP 매핑 기반 친숙명 표시, CSV+DB 이중 저장, 분석 워크스페이스
   * - 통합 (레거시)
     - 8008
     - 일반 RTDE 데모 / current-window 모니터. 런타임 freq·field 변경 + GP write 지원

두 프로세스는 **동시에 띄울 수 있습니다** (서로 다른 포트). 같은 로봇에 연결해도
RTDE recipe 는 독립적이라 충돌하지 않습니다.
