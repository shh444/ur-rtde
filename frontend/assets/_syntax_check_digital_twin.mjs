const THREE={Vector3: class { constructor(x=0,y=0,z=0){this.x=x;this.y=y;this.z=z;} }, Euler: class { constructor(x=0,y=0,z=0,o="XYZ"){this.x=x;this.y=y;this.z=z;this.order=o;} }}; const OrbitControls=class{}; const ColladaLoader=class{}; const STLLoader=class{};

const Y_AXIS = new THREE.Vector3(0, 1, 0);
const FLANGE_RPY = new THREE.Euler(0, -Math.PI / 2, -Math.PI / 2, "XYZ");
const MESH_CONFIG_URL = "/assets/ur_mesh_presets.json";
const DEFAULT_IMPORTED_MESH_SCALE = 0.001; // UR visual meshes are effectively mm-sized in these assets.
const JOINT_CHAIN = [
...
function applyOrigin(group, origin) {
  ...
  group.rotation.set(Number(origin?.roll) || 0, Number(origin?.pitch) || 0, Number(origin?.yaw) || 0, "XYZ");
}
...
export class DigitalTwinView {
  ...
  async cloneCollada(url, linkName) {
    if (!this.assetCache.has(url)) {
      this.assetCache.set(url, this.loader.loadAsync(url).then((result) => normalizeImportedScene(result.scene, linkName)));
    }
  ...
  async addLinkVisual(linkFrame, modelConfig, linkName) {
    ...
    try {
      ...
      wrapper.add(scene);
      this.log("info", "Loaded link visual", { linkName, url: primaryUrl, scale: wrapper.scale.x });
    } catch (error) {
  ...
  async buildVisualModel(modelKey, modelConfig) {
    ...
    for (const item of JOINT_CHAIN) {
      ...
      applyOrigin(jointOrigin, modelConfig?.joints?.[item.joint] || {});
